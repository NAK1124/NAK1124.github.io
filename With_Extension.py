...
EXTENSION (Level 4+):
- Display option includes a MULTI-CRITERIA FILTER (AND logic)
  Users can filter by ANY combination of:
  gender AND lunch AND test preparation course
  (Press Enter to skip a criterion)
"""
...
def display_records(records: list[StudentRecord]) -> None:
    ...
    print("5) Multi-criteria filter (EXTENSION)")

    choice = prompt_int_range("Choose display option (1-5): ", 1, 5)
    ...
    if choice == 2:
        ...
        # EXTENSION: Multi-criteria filtering using AND logic
        print("\nMulti-criteria filter (press Enter to skip a criterion)")
        g = input("Gender (male/female): ").strip().lower()
        l = input("Lunch (standard/free/reduced): ").strip().lower()
        p = input("Prep (none/completed): ").strip().lower()

        filtered = records
        if g:
            filtered = [x for x in filtered if x.gender.lower() == g]
        if l:
            filtered = [x for x in filtered if x.lunch.lower() == l]
        if p:
            filtered = [x for x in filtered if x.prep_course.lower() == p]
...
TEST 2 (Extension – AND Logic Filtering)
Steps:
1) Choose menu option 2 (Display records).
2) Choose option 5 (Multi-criteria filter).
3) Enter gender = female.
4) Enter lunch = standard.
5) Enter test preparation course = completed.
